...
function t2MCfun(AgN_lib,AreaSize,numRedo,dualsubIter,data_mainFolder)
% close all
% clear
% clc
rng(69);

this_scriptName='t2MCfun';%mfilename
...
% areaWidth=1;
% areaHeight=1;
areaWidth=AreaSize(1);
areaHeight=AreaSize(2);
...
for iterRedo=1:numRedo   
    ...
    try
        ...
        [primal_cost,dual_cost,primal_cost_RA,dual_cost_RA, consensus_error,...
           ~,~,ZRA]= twodualsub(dualsubIter,AgN,A,ANW,b,c,g,G,H,LB,UB);
        
        % centralized solution
        [~, centr_cost, ~] = linprog(c,[],[],[H;G],[b;g],LB,UB,linprog_options);
